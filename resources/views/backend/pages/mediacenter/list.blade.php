                            ...
                            <table class="table table-striped" id="table1">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col">Media Center ID</th>
                                        <th scope="col">Media Center Title</th>
                                ...
                                <tbody>
                                    @foreach ($mediacenter as $mediacenter)
                                        <tr>
                                            <td>{{ $mediacenter->id }}</td>
                                            <td>{{ $mediacenter->title }}</td>

                                            <td>{!! html_entity_decode($mediacenter->description) !!}</td>
                                            <td>{{ $mediacenter->videourl }}</td>
                                            <td>{{ $mediacenter->status == '1' ? 'Hidden' : 'Visible' }}</td>
                                            <td><img style="height:50px; width:50px;"
                                                    src="{{ asset('/uploads/mediacenter/' . $mediacenter->image) }}"
                                                    alt=""></td>
                                            <td>
                                                <div style="display: flex; align-items: center;">
                                                    <a href="{{ route('mediacenter-delete', ['id' => $mediacenter->id]) }}"
                                                        onclick="confirmation(event)" style="margin-right: 10px;"><i
                                                            class="bi bi-trash" style="color: red;font-size:20px"></i></a>
                                                    <a href="{{ route('mediacenter-edit', ['id' => $mediacenter->id]) }}"><i
