...
<div class="page-heading">
          ...
          <div class="col-12 col-md-6 order-md-2 order-first">
              <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                  <ol class="breadcrumb">
                      ...
                      <li class="breadcrumb-item active" aria-current="page">Lectures Table</li>
                  </ol>
              </nav>
          </div>
      </div>
  ...
  <section class="section">
      <div class="card">
          ...
          <div class="">
              <a href="{{ route('lecture-create') }}" class="btn btn-success"
                  style="float: right; margin-top: -51px; margin-right: 41px;">Add Lecture</a>
                  ...
                  <table class="table table-striped" id="table1">
                      ...
                      <tbody>
                        @foreach($lectures as $lectures)
                              <tr>
                                <td>{{ $lectures->course->title }}</td>
                                  <td>{{ $lectures->title }}</td>
                                  <td>{!! html_entity_decode($lectures->description) !!}</td>
                                  <td>{{ $lectures->video_link }}</td>

                                  <td>
                                      <div style="display: flex; align-items: center;">
                                          <a href="{{ route('lecture-delete', ['id' => $lectures->id]) }}"
                                              onclick="confirmation(event)" style="margin-right: 10px;"><i
                                                  class="bi bi-trash" style="color: red;font-size:20px"></i></a>
                                          <a href="{{ route('lecture-edit', ['id' => $lectures->id]) }}"><i
...
function confirmation(ev) {
  ...
  swal({
          title: "Are you want to Delete Department",
